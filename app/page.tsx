...
function ScheduleGrid({
  ...
  bestSlots?: TimeSlot[];
  meetingMap?: Record<TimeSlot, string>;
          ...
          {HOURS.map((h, hi) => (
            <tr key={h}>
              ...
              {DAYS.map((_, d) => {
                ...
                const isPriority = priority.includes(s);
                const active = availability.includes(s) || isPriority;
                const inRect = inDragRect(d, hi);
                const title = meetingMap[s];
                ...
                if (meetingOnly) {
                  ...
                  cellClass = "bg-violet-400 border-violet-400 cursor-not-allowed";
                } else if (inRect) {
                  cellClass = drag!.filling
                    ? "bg-primary/60 border-primary/60"
                    : "bg-muted border-border opacity-40";
                } else if (isBest) {
                  cellClass = "bg-sky-400 border-sky-400";
                } else if (isPriority) {
                  cellClass = "bg-amber-400 border-amber-400";
                } else if (active) {
                  cellClass = emerald
                ...
                return (
                  <td key={d} className="p-0.5">
                    <div
                      title={isBooked && title ? title : undefined}
                      className={`h-8 rounded border transition-colors ${cellClass} ${onBatchToggle && !isBooked ? "cursor-pointer" : ""}`}
                      onMouseDown={(e) => {
                        if (!onBatchToggle || isBooked) return;
                        e.preventDefault();
                        dragging.current = true;
                        setDrag({
                          startDay: d, startHourIdx: hi,
                          curDay: d, curHourIdx: hi,
                          filling: !active,
                        });
                      }}
                      onMouseOver={() => {
...
export default function MeetFlow() {
  ...
  const sortedCommonSlots = [...commonSlots].sort(
    (a, b) =>
      members.filter((m) => m.priority.includes(b)).length -
      members.filter((m) => m.priority.includes(a)).length
  );

  // Best slots = all slots tied for the highest priority count
  const topPriorityCount = sortedCommonSlots.length > 0
    ? members.filter((m) => m.priority.includes(sortedCommonSlots[0])).length
    : 0;
  const bestSlots = topPriorityCount > 0
    ? sortedCommonSlots.filter(
        (s) => members.filter((m) => m.priority.includes(s)).length === topPriorityCount
      )
    : [];

  function batchToggleMySlots(slots: TimeSlot[], fill: boolean, mode: "normal" | "priority") {
    setMembers((prev) =>
      prev.map((m) => {
        if (m.id !== "me") return m;
        if (!fill) {
          return {
            ...m,
            availability: m.availability.filter((x) => !slots.includes(x)),
            priority: m.priority.filter((x) => !slots.includes(x)),
          };
        }
        if (mode === "priority") {
          return {
            ...m,
            priority: [...new Set([...m.priority, ...slots])],
            availability: m.availability.filter((x) => !slots.includes(x)),
          };
        } else {
          return {
            ...m,
            availability: [...new Set([...m.availability, ...slots])],
            priority: m.priority.filter((x) => !slots.includes(x)),
          };
        }
      })
        ...
        <Tabs defaultValue="members">
          ...
          <TabsContent value="my-schedule">
            <div className="mb-5">
              ...
              <p className="text-sm text-muted-foreground mt-0.5">選擇模式後，點擊或拖曳來標記時段</p>
            </div>
            <div className="flex gap-2 mb-5">
              <Button
                size="sm"
                variant={fillMode === "normal" ? "default" : "outline"}
              ...
              <Button
                size="sm"
                variant={fillMode === "priority" ? "default" : "outline"}
                className={
                  fillMode === "priority"
                    ? "bg-amber-400 hover:bg-amber-500 text-white border-0"
                    : "border-amber-400 text-amber-600 hover:bg-amber-50"
                }
                onClick={() => setFillMode("priority")}
              >
                ★ 優先空閒
              </Button>
            </div>
            <Card>
              <CardContent className="pt-6">
                ...
                <ScheduleGrid
                  ...
                  onBatchToggle={(slots, fill) => batchToggleMySlots(slots, fill, fillMode)}
                />
              </CardContent>
            </Card>
          </TabsContent>
